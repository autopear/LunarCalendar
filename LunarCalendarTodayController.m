//
//  LunarCalendarTodayController.h
//  LunarCalendarTodayController
//
...
//Controller for iOS 7 & 8
@interface LunarCalendarTodayController: _SBUIWidgetViewController <UIAlertViewDelegate> {
    UIView *_weeView;
...
static void LoadPreferences() {
    ...
    if (preferences) {
        ...
        NSString *readFontName = [preferences objectForKey:@"FontName"] ? [preferences objectForKey:@"FontName"] : @"Helvetica";
        ...
        int readTextAlign = [preferences objectForKey:@"TextAlign"] ? [[preferences objectForKey:@"TextAlign"] intValue] : 0;
        ...
        CGFloat readShadowWidth = [preferences objectForKey:@"ShadowWidth"] ? [[preferences objectForKey:@"ShadowWidth"] floatValue] : 0.0f;
        if (readShadowWidth < -5.0f)
            readShadowWidth = -5.0f;
        if (readShadowWidth > 5.0f)
            readShadowWidth = 5.0f;
        CGFloat readShadowHeight = [preferences objectForKey:@"ShadowHeight"] ? [[preferences objectForKey:@"ShadowHeight"] floatValue] : 0.0f;
...
- (id)init {
    ...
    if ((self = [super init])) {
        ...
        if (kCFCoreFoundationVersionNumber < 1140.10)
            ...
            NSLog(@"Lunar Calendar for Notification Center for iOS 8 loaded.");
	}
	return self;
}
...
- (void)hostWillPresent {
    ...
    if (viewHeightChanged) {
        ...
        if (kCFCoreFoundationVersionNumber < 1140.10)
            [(_SBUIWidgetViewController *)[[self widgetHost] delegate] widget:[self widgetHost] didUpdatePreferredSize:[self preferredViewSize]];
        else
            [(SBWidgetViewController *)[self widgetHost] requestPreferredViewHeightWithHandler:nil];
    }
